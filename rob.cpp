...
bool rob_check_space(ROB *rob)
{
    ...
    if (((rob->tail_ptr - rob->head_ptr) == (NUM_ROB_ENTRIES -1)) || ((rob->tail_ptr + 1) == rob->head_ptr))
    {  
        counter = false;        
    }
    else
...
int rob_insert(ROB *rob, InstInfo inst)
{
    ...
    if (counter)
    {
        rob->entries[rob->tail_ptr] = rob->entries[inst.dr_tag];
        if (rob->tail_ptr == NUM_ROB_ENTRIES && rob->entries[0].valid == false)     
        {
             rob->tail_ptr = 0;
             return inst.dr_tag;
        }
...
bool rob_check_ready(ROB *rob, int tag)
{
    ...
    if (rob->entries[tag].valid == true && rob->entries[tag].ready == true)
    {
        return true;
    }
    else
...
InstInfo rob_remove_head(ROB *rob)
{
    ...
    return rob->entries[rob->head_ptr - 1].inst;
}
